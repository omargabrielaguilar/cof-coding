<form class="grow" action="">
  ...
  <textarea id="{{ $fieldName }}" name="{{ $fieldName }}" class="
    w-full resize-none text-lg px-3 py-2 rounded-md
    border
    border-transparent     {{-- Siempre reserva el espacio border, pero invisible --}}
    bg-pixl-dark text-pixl-light placeholder:text-pixl-light/40
    focus:border-eyes-care hover:border-eyes-care
    focus:outline-none
    transition
  " placeholder="{{ $placeholder }}" @if(! empty($rows)) rows="{{ $rows }}" @endif>{{ old($fieldName) }}</textarea>
  ...
  <div class="flex items-center justify-between gap-4">
    ...
    <button type="submit"
      class="bg-eyes-care hover:bg-eyes-care/90 active:bg-eyes-care/95 text-eyes-care-dark border border-transparent px-4 py-1 text-sm">
      Post
    </button>
  </div>
</form>
